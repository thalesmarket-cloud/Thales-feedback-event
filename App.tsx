    ...
    if (saved) {
      ...
      const dummy: Feedback[] = [
        { 
          ...
          recommendation: 'Oui', contactMe: true, contactEmail: 'j.dupont@tech.com'
        },
        { 
          ...
          recommendation: 'Peut-être', contactMe: false
        }
      ];
      setFeedbacks(dummy);
    ...
    <div className="min-h-screen flex flex-col items-center py-12 px-4 bg-slate-50">
      ...
      <div className="fixed top-4 right-4 z-50">
        <button 
          ...
          className="bg-white/80 backdrop-blur-sm p-2 rounded-full shadow-sm border border-slate-200 text-slate-400 hover:text-[#0076B9] transition-all"
      ...
      {view === 'success' && (
        <div className="max-w-md w-full bg-white rounded-3xl p-12 shadow-2xl text-center animate-in fade-in zoom-in duration-500">
          <div className="w-20 h-20 bg-blue-50 rounded-full flex items-center justify-center mx-auto mb-8">
            <svg className="w-10 h-10 text-[#0076B9]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <button 
            ...
            className="inline-flex items-center justify-center px-8 py-3 bg-[#0076B9] text-white rounded-xl font-semibold hover:bg-[#1CB4E6] transition-all shadow-lg"
          >
            Fermer
          </button>
